...
# read db
# BGQ3_db <- read_rds("BGQ3_db_clean.rds")
BGQ3_db <- read_rds('https://osf.io/cpgzf/download')
names(BGQ3_db)
...
basic_var <- c('accommodates' ,  'bathrooms' ,  'bedrooms_f_1Broom' , 'bedrooms_f_2Broom','bedrooms_f_3Broom' , 'bedrooms_f_4Broom' ,'`bedrooms_f_5+Broom`' , '`room_type_f_Private room`'  )
# '`room_type_f_Shared room`' not present in the data, hence removed
...
# Fit the linear model - whole BGQ3_db
OLS1_BGQ3 <- lm(formula_ols1 , data = BGQ3_db)
OLS2_BGQ3 <- lm(formula_ols2 , data = BGQ3_db)
OLS3_BGQ3 <- lm(formula_ols3 , data = BGQ3_db)
OLS4_BGQ3 <- lm(formula_ols4 , data = BGQ3_db)

# Regression results
#stargazer_r(list(OLS1_BGQ3, OLS2_BGQ3, OLS3_BGQ3, OLS4_BGQ3), float=F, se = 'robust', digits=2, dep.var.caption = "Dep. var: ln price", keep.stat = c("rsq","n"), no.space = T, omit = "^neighbourhood_cleansed", add.lines = list(c("Neighbourhood", "No", "No", "No", "Yes")))
stargazer(OLS1_BGQ3, OLS2_BGQ3, OLS3_BGQ3, OLS4_BGQ3, type = 'text' )

# Comparison model statistics
# evaluation of the models
models <- c("OLS1_BGQ3", "OLS2_BGQ3", "OLS3_BGQ3", "OLS4_BGQ3")
AIC <- c()
BIC <- c()
RMSE <- c()
RSquared <- c()
...
set.seed(1234)
lasso_model_BGQ3 <- caret::train(formula_ols4,
                                 data = BGQ3_db,
                                 method = "glmnet",
...
print(lasso_model_BGQ3$bestTune$lambda)

lasso_coeffs_BGQ3 <- coef(lasso_model_BGQ3$finalModel, lasso_model_BGQ3$bestTune$lambda) %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column(var = "variable") 

print(lasso_coeffs_BGQ3)

# RMSE Lasso
# filter LASSO coeffs > 0
lasso_coeffs_BGQ3_nz <- lasso_coeffs_BGQ3 %>%
  filter(s1!=0)

print(nrow(lasso_coeffs_BGQ3_nz))
# LASSO VARS ---- 
stargazer(lasso_coeffs_BGQ3_nz ,summary = F, type = 'text')

# Evaluate model. CV error:
lasso_cv_rmse_BGQ3 <- lasso_model_BGQ3$results %>%
  filter(lambda == lasso_model_BGQ3$bestTune$lambda) %>%
  dplyr::select(RMSE)
# LASSO LINE HORSE TAB ---- 
print(paste('Lasso RMSE Training set:' , round(lasso_cv_rmse_BGQ3[1, 1], 2), sep = ' ' ))
...
results_rf <- resamples(
  list(
    model_1  = rf_modelBGQ3_1,
    model_2  = rf_modelBGQ3_2,
    model_3 = rf_modelBGQ3_3
...
# again, choice goes for rf_model2 (including neighbourhoods)
...
set.seed(1234)
gbm_modelBGQ3 <- train(formula(paste0("price_ln ~", paste0(predictors_2rf, collapse = " + "))),
                       data = BGQ3_db,
                       method = "gbm",
                       trControl = train_control,
                       verbose = FALSE,
...
gbm_modelBGQ3_2 <- train(formula(paste0("price_ln ~", paste0(predictors_2rf, collapse = " + "))),
                         data = BGQ3_db,
                         method = "gbm",
                         trControl = train_control,
                         verbose = FALSE,
...
final_models <-  list("Random forest (smaller model M3)" = rf_modelBGQ3_1,
                      "Random forest M4" = rf_modelBGQ3_2,
                      "Random forest M4 + interact" = rf_modelBGQ3_3,
                      "GBM (basic tuning) M4"  = gbm_modelBGQ3,
                      "GBM (broad tuning) M4" = gbm_modelBGQ3_2)
...
horsetab_BGQ3_OLS <- eval$RMSE[eval$Model == 'OLS4_BGQ3']
horsetab_BGQ3_OLS <- as.data.frame(horsetab_train_OLS)
row.names(horsetab_BGQ3_OLS) <- 'OLS M4'
colnames(horsetab_BGQ3_OLS) <- "CV RMSE"

horsetab_BGQ3_LASSO <- round(lasso_cv_rmse_BGQ3[1, 1], 3)
horsetab_BGQ3_LASSO <- as.data.frame(horsetab_BGQ3_LASSO)
row.names(horsetab_BGQ3_LASSO) <- 'LASSO M4'
colnames(horsetab_BGQ3_LASSO) <-  "CV RMSE"

horsetab_addon <- rbind(horsetab_BGQ3_OLS, horsetab_BGQ3_LASSO)
...
BGQ3_db$predicted_g2 <- predict(gbm_modelBGQ3_2, newdata = BGQ3_db)
BGQ3_db$predicted_g1 <- predict(gbm_modelBGQ3, newdata = BGQ3_db)

# Scatterplot of Actual vs Predicted Prices BGQ3
BGQ3_GBM_p <-  ggplot(BGQ3_db, aes(y = price_ln)) +
                  ...
                  scale_color_manual(name = "Models", 
                     values = c("GBM broad" = "aquamarine4", "GBM basic" = "blue4")) +
                  labs(title = "Actual vs Predicted Prices BGQ3",
...
ggsave( 'GBM_baseBGQ3.png', plot = BGQ3_GBM_p, width = 6 , height = 6)
